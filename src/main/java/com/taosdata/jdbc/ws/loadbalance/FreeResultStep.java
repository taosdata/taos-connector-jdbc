...
class FreeResultStep implements Step {
    ...
    public CompletableFuture<StepResponse> execute(BgHealthCheck context, StepFlow flow) {
        ...
        if (!context.getWsClient().isOpen() || context.getResultId() < 0) {
        ...
        return CompletableFuture.completedFuture(new StepResponse(StepEnum.QUERTY, context.getNextInterval())); // 立即返回，等待连接建立
    }
}