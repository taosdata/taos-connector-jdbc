...
public class WSConsumerMetaTest {
  ...
  public void testAutoCreateChildTable() throws Exception {
    ...
    try (TaosConsumer<Map<String, Object>> consumer = new TaosConsumer<>(properties)) {
      ...
      consumer.poll(Duration.ofMillis(100));
      {
        ...
        String sql =
            String.format(
                "insert into %s using %s.%s tags(%s, '%s', %s) values (now, 1)",
                "act" + idx, DB_NAME, SUPER_TABLE, idx, "t" + idx, "true");
        statement.execute(sql);
      }
  ...
  public void testCreateChildTableJson() throws Exception {
    ...
    // create topic
    statement.executeUpdate(
        "create topic if not exists " + topic + " only meta as STABLE " + SUPER_TABLE_JSON);
    ...
    try (TaosConsumer<Map<String, Object>> consumer = new TaosConsumer<>(properties)) {
      ...
      consumer.poll(Duration.ofMillis(100));
      {
        ...
        String sql =
            String.format(
                ...
                "et" + idx, DB_NAME, SUPER_TABLE_JSON, "{\"a\":1}");
        statement.execute(sql);
      }
      ...
      List<Tag> tags = new ArrayList<>();
      tags.add(
          new Tag("t1", 15, JsonUtil.getObjectMapper().getNodeFactory().textNode("{\"a\":1}")));
      dstMeta.setTags(tags);
      dstMeta.setCreateList(new ArrayList<>());
    ...
    try (TaosConsumer<Map<String, Object>> consumer = new TaosConsumer<>(properties)) {
      ...
      consumer.poll(Duration.ofMillis(100));
      {
        ...
        for (int idx = 1; idx <= subTableNum; idx++) {
          sql +=
              String.format(
                  " %s using %s.%s tags(%s, '%s', %s)",
                  "dt" + idx, DB_NAME, SUPER_TABLE, idx, "t" + idx, "true");
        }
        statement.execute(sql);
      }
  ...
  public void testCreateNormalTable() throws Exception {
    ...
    try (TaosConsumer<Map<String, Object>> consumer = new TaosConsumer<>(properties)) {
      ...
      consumer.poll(Duration.ofMillis(100));
      {
        int idx = 1;
        statement.execute(
            "create table if not exists " + normalTable + idx + " (ts timestamp, c1 int)");
      }
      ...
      consumer.poll(Duration.ofMillis(100));
      {
        ...
        for (int idx = 1; idx <= subTableNum; idx++) {
          sql +=
              String.format(
                  " %s using %s.%s tags(%s, '%s', %s)",
                  "cht" + idx, DB_NAME, SUPER_TABLE, idx, "t" + idx, "true");
        }
        statement.execute(sql);
